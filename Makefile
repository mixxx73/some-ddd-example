...
lint-yaml-configs:
	@$(PHP_CONT) bin/console lint:yaml config/

## -- Fixers 🔧 ----------------------------------------------------------------
fixers: phpcbf phpcsfixer ## run all code fixers
...
tests-failing: tests-unit-failing ## run all tests with @group failing

tests-unit: ## run unit tests only
	@$(PHP_CONT) vendor/bin/phpunit

tests-unit-failing: ## run unit tests only with @group failing
	@$(PHP_CONT) vendor/bin/phpunit --group failing
